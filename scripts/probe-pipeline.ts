...
async function main() {
  const facePath = resolve("public/creators/creator-4.png");
  const faceMimeType = facePath.toLowerCase().endsWith(".jpg") || facePath.toLowerCase().endsWith(".jpeg")
    ? "image/jpeg"
    : "image/png";

  // food-ad (23.4s comedic restaurant ad — panicked then pleased, 2 shots)
  // + creator-4 + NO user product. Note: 23s exceeds Seedance's 15s max so
  // the orchestrator will clamp to 15s.
  const run = createRun({
    template_id: "food-ad",
    looks: [
      { product_ids: [] },
